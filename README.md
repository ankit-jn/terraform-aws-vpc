...
Refer [Configuration Examples](https://github.com/ankit-jn/terraform-aws-examples/tree/main/aws-vpc) to get an idea of effectively utilizing this module for VPC Provisioning
...
|:------|:------|:------|:------|:------:|:------|
| <a name="create_vpc"></a> [create_vpc](#input\_create\_vpc) | Flag to decide if a new VPC should be created. | `bool` | `true` | no | |
| <a name="vpc_id"></a> [vpc_id](#input\_vpc\_id) | The ID of the VPC; Required only whern `create_vpc` is set false | `string` |  | no | |
...
| <a name="vpc_base_configs"></a> [vpc_base_configs](#vpc\_base\_configs) | Basic Configuration Map for VPC | `map` | `{}` | no | <pre>{<br>   "enable_ipv6"         = true<br>   "instance_tenancy"    = "default"<br>   "enable_dhcp_options" = false<br>}<pre>|
| <a name="vpc_ipam_configs"></a> [vpc_ipam_configs](#vpc\_ipam\_configs) | Configuration Map for IPAM | `map` | `{}` | no | <pre>{<br>   "use_ipv4_ipam_pool"  = "10.0.0.0/16"<br>   "ipv4_ipam_pool_id"   = "ipam-xxxx"<br>   "ipv4_netmask_length" = 28><br>}<pre>|
| <a name="vpc_dns_configs"></a> [vpc_dns_configs](#enable\_vpc\_dns\_configs) | Configuration Map for DNS Support | `map` | <pre>{<br>   enable_dns_support = true<br>   vpc_dns_host_name  = false<br>}<pre> | no | <pre>{<br>   enable_dns_support = true<br>   vpc_dns_host_name  = false<br>}<pre> |
| <a name="vpc_classiclink_configs"></a> [vpc_classiclink_configs](#vpc\_classiclink\_configs) | Configuration Map for CLassic Link | `map` | <pre>{<br>     enable_classiclink             = true<br>   enable_classiclink_dns_support = false<br>}<pre> | no | <pre>{<br>   enable_classiclink             = true<br>   enable_classiclink_dns_support = false<br>}<pre> |
| <a name="vpc_secondary_cidr_blocks"></a> [vpc_secondary_cidr_blocks](#vpc\_secondary\_cidr\_blocks) | Configuration Map for Secondary CIDR blocks  | `map` | `{}` | no | <pre>{<br>   "CIDR-1"   = {<br>      cidr_block = "x.x.x.x/xx<br>   }<br>}<pre> |
...
| Name | Description | Type | Default | Required | Example|
|:------|:------|:------|:------|:------:|:------|
| <a name="nat_gateways"></a> [nat_gateways](#input\_nat\_gateways) | The configuration map of Nat Gateways.<br>Each key will be unique identifier for the Nat Gateway<br> Value will be the subnet name where NAT gateway will be provisioned | `map` | `{}` | no | <pre>{<br>   "nat-1" = "\<subnet-name\>"<br>   "nat-2" = "\<subnet-name\>"<br>}<pre> |
...
| <a name="subnets"></a> [subnets](#output\_subnets) | `map` | The configuration of all subnets:<br><b>Map Key:</b> Subnet name<br><b>Map Value:</b> Map of the following Subnet attributes:<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`id` - The ID of Subnet<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`arn` - Amazon Resource Name (ARN) of Subnet<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`availability_zone` - AZ Name where subnet is provisioned.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`availability_zone_id` - AZ ID where subnet is provisioned.|
| <a name="route_table_id"></a> [route_table_id](#output\\_route\_table\_id) | `string` | ID of route table |
| <a name="nat_gatways_config"></a> [nat_gatways_config](#output\\_nat\_gatways\_config) | `map` | The configuration of all NAT Gateways:<br><b>Map Key:</b> NAT Gateway name<br><b>Map Value:</b> Map of the following NAT Gateway attributes:<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`id` - The ID of the NAT Gateway.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`allocation_id` - The Allocation ID of the Elastic IP address for the gateway.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`network_interface_id` - The ENI ID of the network interface created by the NAT gateway.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`private_ip` - The private IP address of the NAT Gateway.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`public_ip` - The public IP address of the NAT Gateway.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`subnet_id` - The ID of the subnet in which NAT Gateway is provisioned.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`subnet_name` - The name of the subnet in which NAT Gateway is provisioned. |
...
Module is maintained by [Ankit Jain](https://github.com/ankit-jn) with help from [these professional](https://github.com/ankit-jn/terraform-aws-vpc/graphs/contributors).
